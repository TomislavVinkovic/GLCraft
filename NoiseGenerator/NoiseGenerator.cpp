...
NoiseGenerator::NoiseGenerator(int seed)
        : m_seed(seed)
{
    m_noiseParameters.octaves = 8;
    m_noiseParameters.amplitude = 80;
    m_noiseParameters.smoothness = 350;
...
noexcept
{
         ...
         a++) // This loops through the octaves.
    {
        ...
        totalValue +=
                noise(((double)x) * frequency / m_noiseParameters.smoothness,
                      ((double)z) * frequency / m_noiseParameters.smoothness) *
                amplitude;
    }
...
int NoiseGenerator::getWaterLevel() {
    return waterLevel;
}
